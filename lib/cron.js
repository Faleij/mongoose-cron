...
module.exports = class Cron extends events.EventEmitter {
  ...
  */
  ...
  getNextStart(doc) {
    ...
    const future = new Date(Date.now() + this._nextDelay); // date when the next start is possible
    const start = new Date(doc.cron.startAt);
    ...
    try {
      const options = {
        currentDate: future,
        endDate: doc.cron.stopAt || undefined
      };
      const tz = doc.cron.timezone && String(doc.cron.timezone).trim();
      if (tz) options.tz = tz;
      const interval = cronParser.parseExpression(doc.cron.interval, options);
  ...
  _handleError(err, doc) {
    ...
    switch(err.name) {
      case 'CronNoDocumentError':
        ...
        break;
      default:
        if (doc) promise = promise.then(res => doc.update({$unset: {'cron.enabled': 1, 'cron.locked': 1}, 'cron.lastError': err.message}));
        this._model.emit('mongoose-cron:error', err, doc);
    }
    return promise.then(res => this._nextTick(delay || this._tickDelay))
