  ...
  */
  ...
  getNextStart(doc) {
    ...
    try { // new date
      ...
      let next = later.schedule(schedule).next(1, future.toDate(), doc.cron.stopAt);
      // later returns a single Date when count=1, or later.NEVER (0) when no occurrences
      if (!next || next === later.NEVER || !(next instanceof Date)) {
        return null;
      }
      return next;
    } catch (err) {
